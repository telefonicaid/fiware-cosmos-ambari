...
class Bootstrap(threading.Thread):
  ...
  ACTION_SCRIPT_FILENAME = "actionAgent.py"
  ...
  def getRunActionWithPasswordCommand(self, expected_hostname):
    setupFile = self.getRemoteName(self.ACTION_SCRIPT_FILENAME)
  ...
  def getRunActionWithoutPasswordCommand(self, expected_hostname):
    setupFile=self.getRemoteName(self.ACTION_SCRIPT_FILENAME)
    passphrase=os.environ[AMBARI_PASSPHRASE_VAR_NAME]
    server=self.shared_state.ambari_server
    version=self.getAmbariVersion()
  ...
  def getRunActionCommand(self, expected_hostname):
    if self.hasPassword():
      return self.getRunActionWithPasswordCommand(expected_hostname)
    else:
      return self.getRunActionWithoutPasswordCommand(expected_hostname)
...
class SharedState:
  def __init__(self, user, sshkey_file, script_dir, boottmpdir, action_agent_file,
    ...
    self.action_agent_file = action_agent_file
...
def main(argv=None):
    ...
    if len(onlyargs) < 10:
        sys.stderr.write("Usage: <comma separated hosts> <tmpdir for storage> "
            ...
            "<cluster os type> <ambari version> <ambari port> <password_file>\n")
        sys.exit(2)
    # Parse the input
    ...
    password_file = onlyargs[9]

    # ssh doesn't like open files
    stat = subprocess.Popen(["chmod", "600", ssh_key_file], stdout=subprocess.PIPE)

    if password_file is not None and password_file != 'null':
